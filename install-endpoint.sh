...
if which dnf >/dev/null 2>&1; then
    dnf install -y epel-release perl audit audispd-plugins setools-console >/dev/null 2>&1
elif which yum >/dev/null 2>&1; then
    yum install -y epel-release perl audit audispd-plugins setools-console >/dev/null 2>&1
else
...
echo ""
echo "Setting up audisp-simplify baseline"
echo "Props to mtkirby for this: https://github.com/mtkirby/audisp-simplify"
...
cp td-agent-endpoint.conf /etc/td-agent/td-agent.conf

if [ -z "$TARGETIP" ]; then
	echo "No OpenSIEM Server specified"
	echo "Please edit host output in <MATCH>" 
else
	sed -i -e 's/host 192.168.1.3/host '$TARGETIP'/g' /etc/td-agent/td-agent.conf
fi
